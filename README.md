...
Live domain: **[newcastleramps.com.au](https://newcastleramps.com.au)** (all Google Ads
...
→ add **newcastleramps.com.au** (Cloudflare manages DNS automatically if the zone is
on Cloudflare). `site` in `astro.config.mjs` is already set to it.

**Redirect hunterramps.au → newcastleramps.com.au:** Workers static assets read
...
`https://newcastleramps.com.au${http.request.uri.path}`, status **301**, preserve
query string.

### Cloudflare Pages (alternative)
1. Connect the repo. **Build command:** `npm run build`. **Output dir:** `dist`.
2. Add custom domain **newcastleramps.com.au** in *Pages → Custom domains*. The
   `public/_redirects` (incl. the hunterramps.au→newcastleramps.com.au rule) and
   `public/_headers` files are applied automatically.

### Netlify
1. New site from Git. **Build:** `npm run build`. **Publish:** `dist`.
2. *Domain settings* → add **newcastleramps.com.au** → point DNS (A/ALIAS or Netlify DNS).
3. `_redirects` / `_headers` are picked up automatically from `public/`.

### Vercel
1. Import the repo (Astro auto-detected). **Output:** `dist`.
2. *Settings → Domains* → add **newcastleramps.com.au** and follow the DNS steps.
...
- [ ] `site` in `astro.config.mjs` = `https://newcastleramps.com.au` (already set).
- [ ] Point **newcastleramps.com.au** at the deploy; redirect **hunterramps.au** to it.
